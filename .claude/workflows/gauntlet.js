...
const SCORE = {
  ...
  properties: {
    ...
    verdict: { type: 'string' },
    feedback: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, stillHit: { type: 'boolean' }, why: { type: 'string' } }, required: ['id', 'stillHit', 'why'] } },
  },
  required: ['module', 'score', 'errors', 'pass', 'strengths', 'issues', 'verdict', 'feedback'],
}
const FIX = {
  type: 'object',
  properties: {
...
PLAYTEST: read docs/FEEDBACK.md (a real player's 2026-09-14 playtest, items F1–F16, each naming its owning module) and docs/feedback/2026-09-14-levers.md. For EVERY item that names your module, state in the \`feedback\` array whether a player would STILL hit it on the current tree (stillHit) and why, citing the measurement you used (a sim log line, a screenshot, a code path). A ticked item ([x]) is a claim to verify, not a fact. An open P1 item your module owns that a player would still hit caps the score below 8.5; do not tick or edit FEEDBACK.md yourself.`
