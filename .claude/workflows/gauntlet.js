...
const critics = (evidence) => ({
  ...
  upgrades: `You are a game-design critic for the game "Metropolis" (${ROOT}). ${RUBRIC}\nModule under review: src/upgrades/. Read its code, docs/DESIGN.md, and logs/sim-gauntlet.json. Judge: count and spread across the cost ladder (is there always a next upgrade within a few minutes of income?), effect legibility in desc text, meaningfulness (>=+25% or an unlock), unlock conditions that actually trigger (check sim: how many upgrades were bought by 1h/6h/12h), fault isolation (effects mutate mods only), errors. Score module 'upgrades'.\n\nEvidence summary:\n${evidence}`,
  simulation: `You are a systems critic for the game "Metropolis" (${ROOT}). ${RUBRIC}\nModule under review: src/simulation/ (+ its use of src/resources/). Read the code and logs. Judge: correctness of integration (dt, clamps), milestone cadence and rewards, prestige loop design (does legacy make replays faster and meaningful? is threshold sane?), tap action, dashboard gates, no per-tick allocations beyond mods, tick time (logs/gauntlet.json tickStats: must be avg<0.5ms p99<4ms), NaN safety, event emission for UI, code clarity. Score module 'simulation'.\n\nEvidence summary:\n${evidence}`,
  ...
  balance: `You are a game-balance critic for the game "Metropolis" (${ROOT}). ${RUBRIC}\nModule under review: src/balance/config.js and the resulting pacing. Read docs/DESIGN.md pacing targets, then logs/sim-gauntlet.json samples and logs/gauntlet.json samples. Compute and report: time to first building/upgrade/1k pop/10k pop, first prestige availability and when the bot took it, income at 10/30/60/180 min, income growth factor per hour before first prestige, brownout share, cycles count in 12h and cycle durations (should shrink), never-purchased items. Judge against Cookie Clicker/Paperclips pacing: always something to buy within ~1-3 min, no dead stretches > 5 min early, prestige cadence accelerating. Score module 'balance'.\n\nEvidence summary:\n${evidence}`,
})
