...
// The phone breakpoint, as the stylesheet draws it (the population chip is narrowest there).
const NARROW_QUERY = '(max-width: 700px)';

export function createTopbar(ui) {
  const { game } = ui;
  // '303,455 of 303,455 housing' wrapped to three lines inside the chip on a 390px phone;
  // under this query the chip prints the short form '303K / 303K homes' instead (text.js).
  ...
  function update(dt) {
    ...
    setText(popChip.value, line.count);
    setText(popChip.sub, line.sub);
